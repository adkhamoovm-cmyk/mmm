...
# I will just replace the entire promo block with a clean one
start_idx = content.find('# --- PROMO CODES ---')
    ...
    async with async_session() as session:
        ...
        for promo in promos:
            text = f"🎁 Kod: <b>{promo.code}</b>\n💰 Summa: {promo.amount:,.0f} UZS\n" \
                   ...
                   f"🟢 Faol: {'Ha' if promo.is_active and promo.used_count < promo.limit else 'Yoq (Limit to\\'lgan)'}\n\n"
...
with open("bot/handlers/admin.py", "w") as f:
    f.write(clean_content + new_handlers)
